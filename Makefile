PROJECT_NAME := Pulumi PVE Provider
...
SCHEMA_FILE     := provider/cmd/pulumi-resource-pve/schema.json
...
test_provider: tidy_provider
	cd provider && go test -v -count=1 -cover -timeout 2h -parallel ${TESTPARALLELISM} -coverprofile="coverage.txt" ./...
...
sign-goreleaser-exe-%: bin/jsign-6.0.jar
	...
	if [[ "${SKIP_SIGNING}" != "true" ]]; then \
		if [[ "|${AZURE_SIGNING_CLIENT_ID}|${AZURE_SIGNING_CLIENT_SECRET}|${AZURE_SIGNING_TENANT_ID}|${AZURE_SIGNING_KEY_VAULT_URI}|" == *"||"* ]]; then \
			...
			file=dist/build-provider-sign-windows_windows_${GORELEASER_ARCH}/pulumi-resource-pve.exe; \
